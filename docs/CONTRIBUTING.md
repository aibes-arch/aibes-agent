...
感谢你对 `minagent` 的关注！本指南帮助你快速参与项目贡献。
...
- 工具 `call()` 必须是 `async def`。
- IO 操作使用 `asyncio` 原生接口或 `httpx.AsyncClient`。
...
```bash
black --check minagent tests examples
mypy minagent
pytest tests/ -q
```

4. **更新文档**：如果修改了架构、接口或新增工具，同步更新 `docs/`。
5. **提交 commit**：commit message 用中文或英文均可，要求清晰描述改动。
...
| 改动类型 | 需要更新的文档 |
|---------|--------------|
| 新增/修改核心类接口 | [API 参考](./API_REFERENCE.md)、[模块设计](./MODULE_DESIGN.md) |
...
如果你想为 `minagent` 贡献一个新工具，请按以下清单检查：

- [ ] 继承 `Tool` 并实现 `call()`
- [ ] 定义清晰的 `name`、`description`、`input_model`
- [ ] 实现 `is_read_only()`（影响并发策略）
- [ ] 完善的错误处理，返回 `ToolResult.fail(...)`
- [ ] 控制输出长度，避免过大内容冲击上下文
- [ ] 添加到 `minagent/tools/__init__.py`
- [ ] 添加到 `minagent/__init__.py`（可选）
- [ ] 在 `tests/` 中添加单元测试
- [ ] 更新文档
