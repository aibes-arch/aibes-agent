...
在 `minagent` 中，**工具（Tool）** 是 Agent 与外部世界交互的接口。Agent 本身不会直接读文件、执行命令或访问网络，它只能通过调用工具来完成这些操作。
...
- `is_read_only()`：影响并发策略和权限判断。
- `is_concurrency_safe()`：默认等于 `is_read_only()`，复杂工具可单独控制。

---

## 4. 输入模型设计

使用 Pydantic `BaseModel` 定义输入，每个字段都要写 `description`，这是 LLM 理解参数的关键。
...
1. **字段名用英文小写 + 下划线**，与 LLM 常见风格一致。
2. **必填字段用 `...`**，可选字段给默认值。
3. **description 要写清楚单位、格式、约束**。
4. **枚举值说明**：如果字段是枚举，列出可选值。
...
1. **不要让异常抛出**：`ToolRegistry` 会捕获 `call()` 中的异常，但最好自己处理。
2. **错误信息要具体**：包含文件名、命令、原因。
3. **敏感信息不要返回**：比如 API Key、密码。
...
`PermissionEngine` 会检查 `filesystem` / `shell` / `tool` 规则。工具参数中应包含可被检查的关键字段：

- 文件操作：`file_path` 或 `path`
- Shell 操作：`command`
...
- [ ] 工具类文件放在 `minagent/tools/` 下或单独的领域包中
- [ ] 在 `minagent/tools/__init__.py` 中导出
- [ ] 在 `minagent/__init__.py` 中导出（如果希望用户直接 `from minagent import`）
- [ ] 在 `ToolRegistry` 中注册
- [ ] 添加单元测试到 `tests/`
...
- 文件读取：默认 500 行，支持 `offset`/`limit`
- 搜索：默认 250 条结果
- HTTP：默认截断到 2000 字符

### 11.3 参数命名一致

- 文件路径用 `file_path` 或 `path`
- 命令用 `command`
- 超时用 `timeout`
...
如果多个工具调用需要共享状态，使用 `context.set()` 和 `context.get()`，而不是全局变量。
